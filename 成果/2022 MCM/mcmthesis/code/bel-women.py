...
def totalObject(P0):
    ...
    def subObject(l,r,n,P0):
        def func(x,s):
            alpha_x=alpha(x+l)
            return (-P_(x+l)/m*s**4*(np.cos(alpha_x))**2+k/(m*np.cos(alpha_x))*s+s*(g*s**2-y__(x+l))*(Crr+np.tan(alpha_x)*np.cos(2*alpha_x)))
        y0=[0.4]
        y3 = integrate.solve_ivp(func,t_span=(0,r-l), y0=y0, method='RK45', t_eval=np.linspace(0,r-l,n)) 
...
# min=1e4
# count =0
# for i in range(int(1e5)):#totalObject(start)>60*54):
#     count+=1
#     start=np.full(k_*100,550)
#     for i in range(k_*100):
#         start[i]+=np.random.rand()*150
#     score=totalObject(start)
#     if(score<min):
#         min=score 
#         res=start 
#     if(count%100==0):
...
# while(totalObject(start)>60*54):
#     count+=1
#     start=np.full(k_*100,500)
#     for i in range(k_*100):
#         start[i]+=np.random.rand()*100
#     # if (totalObject(start)<4e4):
#     #     res+=[start]
#     #     print(res)
#     if(count%100==0):
#         print('count',count)
